...
実験用安定化電源は、5V source sagと機体側負荷を分けるbench対照としてだけ使える。配備設計として意味のある次の枝は、次のように電力経路へ別の機能を足すことにある。
...
- source sagの因果分離が必要になった場合だけ、実験用5V安定化電源をbench対照として使う。配備状態とは混同しない。
