<!-- STATUS: PENDING | 理由: Season 4ハード設計へ持ち越し。Season 3本線では優先度衝突、OBS/WebUI/GPIO/ストロボ実機確認待ち、単騎出撃時の扱い未定 | 復帰条件: Season 4でキャリブ用ハード構成、絶縁回路、安全確認手順、デーモン同居方式を確定 -->
...
**現在の扱い:** Season 4 ハード設計へ持ち越し。

この仕様は、マルチカメラ収録品質を上げる価値は高いが、GPIOリレー、ストロボ同期、
音声クリック、OBSオフセット書き込み、既存デーモンとの同居をまとめて扱うため、
Season 3の飛行中UX・FAN800統合・単騎出撃検証からは切り離す。

Season 4では「キャリブレーション用ハード治具」として、ガルバニック絶縁回路、
ストロボ/クリック信号、デーモン停止なしの測定経路、安全確認手順を先に固めてから
ソフト実装へ戻す。
