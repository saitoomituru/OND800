...
## [FACT] 復旧結果 2026-07-20

- ユーザーが破損HyperPixelを物理パージした後、NDI sourceの復帰を受信側で確認した。
- callback修正版を配置した実機を再bootすると、systemd serviceはC922とEMEET C960を起動時に検出した。
- 両カメラはMJPG 1920x1080@30fpsで送信し、各3000 frames時点でdrops 0、connections 1だった。
- 2カメラ送信中の温度は56.5〜59.3°C、throttle bitsは`0x0`、今回bootの低電圧logは0件だった。
- Wi-Fiは未復旧であり、SSHとNDIはEthernetで動作した。
- 物理液晶がなくても`/dev/fb0`は残り、現行streamerはdisplay branchも実行した。
- ログ収集中に物理USB抜けで一時的にSSH不能になったが、再接続後に復活した。Pi内部障害としては扱わない。
- 同一bootの追観測では20:06〜20:45に10回の低電圧が再発し、20:45のthrottle bitsは履歴`0x50000`になった。初期の`0x0`は短時間スナップショットであり、電源安定性の確定ではなかった。
- 20:45時点でもWi-Fi radioとSSID scanは動作したが、保存済みWi-Fi profileは存在せず、パージ後に設定が戻った事実はない。

## [INTERPRETATION] 現在の閉じ方

- 最低配信機材としての「Ethernet + OND800 Pi 5 + USBカメラ2台 + NDI受信側」は復旧した。
- NDI自動起動問題は、callback修正、再boot、起動時カメラ検出を含む現在構成で解消している。
- 液晶パージとNDI復旧には時間的相関がある。一方、低電圧は後続窓で再発したため、電圧正常化を液晶パージへ帰属させない。液晶リーク電流も未測定である。
- Wi-Fi、音声、SaaSへの直接送出は未復旧または未実装であり、今回の成功へ含めない。
- 詳細な電力比較は`notes/2026-07-20_post-breakage-electrical-survey.md`を正とする。

## 工学判定と再現試験の停止

- 目視物理破損したHyperPixelをHATアッシー単位でパージし、その後にNDI 2系統が復旧したため、工学的にはHyperPixel HATアッシー不具合としてfault isolationを閉じる。電圧状態は後続窓で再発しており、この判定根拠へ含めない。
- 液晶内部のリーク電流や部分短絡という具体的故障モードは未測定であり、その内部原因までは断定しない。
- 科学的な単変数再現であれば、破損液晶を再装着して再bootする比較が考えられる。しかし、復旧済み母機へ既知の物理破損部品を戻すリスクを負う必要はない。
- 本タスクの目的は原因科学の完成ではなく、最低配信機材への工学的復旧である。再装着試験は停止し、パージ状態を運用構成として採用する。
