...
| 上流AC／AC-DC電源／USB-Cケーブル | HATパージ後も周期的低電圧が継続 | 関与濃厚。経路内のどこかは未分離 |
...
- 上流AC／AC-DC電源／USB-Cケーブル経路の電圧余裕不足: `関与濃厚`。区間未分離
- USBカメラ合計負荷: `関与疑い`
...
1. 現在のパージ状態を維持する。30分追観測で低電圧再発を確認済みのため、次は既知正常なPi 5対応電源／短いUSB-Cケーブルで同一負荷を比較する。
