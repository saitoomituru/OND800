...
工学判定更新: 物理破損を目視できるHyperPixel HATアッシーをパージした後、母機は2カメラNDI全負荷で復旧し、低電圧を観測しなかった。漏れ電流または部分短絡という内部故障モードの直接測定は未実施だが、交換可能モジュール単位の故障隔離としてHyperPixel HATアッシーを黒と判定する。
...
- パージ後は2カメラ1080p30 NDI送信中にthrottle bits `0x0`、低電圧log 0、各3000 framesでdrop 0を観測した。
...
| 寄与候補 | 根拠 | 現在の扱い |
|---|---|---|
| リーク電流／部分短絡／電気的追加負荷 | パージ前は低電圧反復、パージ後はNDI 2系統負荷でも`0x0` | 主要寄与候補。電流値は未測定 |
| airflow／放熱変化 | panel除去で物理流路が変化しうる | 未判定 |
| GPIO／DPI接触、機械応力、ケーブル荷重 | 物理破損HATと接続経路を同時に除去 | 未判定 |
...
電流値そのものは測定していない。観測できたのは低電圧logとthrottle状態の改善であるため、電気的追加負荷を主要寄与候補とするが、具体的な漏れ電流値は主張しない。
